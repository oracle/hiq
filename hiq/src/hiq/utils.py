...
### Plot Gantt-chart for tau
def create_gantt_chart_time(data, fig_path=None):
    import tree
    import pandas as pd
    ...
    def create_df(data):
        ...
        def dfs(result, pass_name, num, node):
            if pass_name == '':
                ...
                result.append([str(pass_name) + str(node.name.strip('_')) + "." + str(num), str(node.name.strip('_')), s, e])
            else:
                if node.start == float('inf'):
                    # start time
                    s = ''
        ...
        def get_color():
            hex_number = ["#" + ''.join([random.choice('ABCDEF0123456789') for i in range(6)])]
            return hex_number[0]
    ...
    for i in range(fl):
        ...
        legend_elements = [Patch(facecolor=c_dict[i], label=i.strip('_')) for i in c_dict]
        ...
        xticks_labels = pd.date_range(proj_start, end=df.End.max(), periods=len(xticks)).floor('ms').format('.%3f')[1:]
        ...
        ax.title.set_text('Request-' + str(i + 1))
...
### Plot Gantt-chart for memory
def create_gantt_chart_memory(data, fig_path=None):
    import tree
    import pandas as pd
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    def create_df(data):
        ...
        def dfs(result, pass_name, num, node, r_min, r_max):
            if pass_name == '':
                ...
                result.append([str(pass_name) + str(node.name.strip('_')) + "." + str(num), str(node.name.strip('_')), s, e, m])
            else:
                if node.name == 'None':
                    s = ''
                    e = ''
        ...
        for i in range(total_data):
            ...
            pass_name = 'Request-' + str(i + 1)
            dfs_result = []
            dfs_result, r_min, r_max = dfs(dfs_result, pass_name, 0, t.root, float('inf'), 0)
            dfs_result = dfs_result[1:]
        ...
        # Create dfs
        def create_dfs_from_list(req_result):
            df = pd.DataFrame(req_result, columns=['Name', 'Group', 'Start', 'End', 'Memory'])
